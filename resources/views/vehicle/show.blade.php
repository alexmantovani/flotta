        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                @php
                    ...
                    $totalMaintenances =
                        $upcomingMaintenances->count() + $inProgressMaintenances->count() + $pastMaintenances->count();
                @endphp
                    ...
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                        ...
                        <div class="overflow-y-auto max-h-[600px]">
                            @if ($groupedReservations->isNotEmpty())
                                @php
                                    // Flatten all reservations to detect consecutive same-driver bookings
                                    $allReservations = $groupedReservations->flatten();
                                @endphp
                                    ...
                                    <tbody
                                        ...
                                        @foreach ($groupedReservations as $month => $reservations)
                                            <tr class="bg-gray-50 dark:bg-gray-900">
                                                <td class="px-3 py-4" style="width: 40px;"></td>
                                            ...
                                            @foreach ($reservations as $index => $reservation)
                                                @php
                                                    // Find current reservation in all reservations
                                                    $currentIndex = $allReservations->search(
                                                        fn($r) => $r->id === $reservation->id,
                                                    );
                                                    $nextReservation =
                                                        $currentIndex !== false &&
                                                        $currentIndex < $allReservations->count() - 1
                                                    ...
                                                    // Check if next reservation is consecutive and same driver
                                                    $isConnected = false;
                                                    if ($nextReservation) {
                                                        $daysDiff = $reservation->date->diffInDays(
                                                            $nextReservation->date,
                                                        );
                                                        $isConnected =
                                                            $nextReservation->driver_id === $reservation->driver_id &&
                                                            $daysDiff <= 1;
                                                    }

                                                    // Check if previous reservation is consecutive and same driver
                                                    $isConnectedToPrev = false;
                                                    if ($prevReservation) {
                                                        $daysDiff = $prevReservation->date->diffInDays(
                                                            $reservation->date,
                                                        );
                                                        $isConnectedToPrev =
                                                            $prevReservation->driver_id === $reservation->driver_id &&
                                                            $daysDiff <= 1;
                                                    }

                                                    // Determine connection type
                                                    $connectionType = 'none';
                                                    if ($isConnectedToPrev && $isConnected) {
                                                        $connectionType = 'middle';
                                                    } elseif ($isConnectedToPrev && !$isConnected) {
                                                        $connectionType = 'end';
                                                    } elseif (!$isConnectedToPrev && $isConnected) {
                                                ...
                                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                                    ...
                                                    <td class="px-3 py-0 whitespace-nowrap relative"
                                                        style="width: 40px;">
                                                        <div class="flex justify-center items-center absolute inset-0">
                                                            @if ($connectionType === 'start')
                                                                <!-- Inizio serie: cerchio + linea verso il basso -->
                                                                <div
                                                                    class="relative h-full w-full flex flex-col items-center">
                                                                    <div class="flex-1"></div>

                                                                    <!-- Barra sotto con pallino allineato in basso -->
                                                                    <div
                                                                        class="flex-1 w-3 bg-indigo-500 dark:bg-indigo-400 rounded-t-full flex justify-center items-start pt-1">
                                                                        <div
                                                                            class="w-1 h-1 rounded-full bg-white dark:bg-indigo-800 z-10">
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            @elseif ($connectionType === 'middle')
                                                                <!-- Mezzo serie: linea continua -->
                                                                <div
                                                                    class="h-full w-3 bg-indigo-500 dark:bg-indigo-400">
                                                                </div>
                                                            @elseif ($connectionType === 'end')
                                                                <!-- Fine serie: linea dall'alto + cerchio -->
                                                                <div
                                                                    class="relative h-full w-full flex flex-col items-center">
                                                                    <div
                                                                        class="flex-1 w-3 bg-indigo-500 dark:bg-indigo-400 rounded-b-full flex justify-center items-end pb-1">
                                                                        <div
                                                                            class="w-1 h-1 rounded-full bg-white dark:bg-indigo-800 z-10">
                                                                        </div>
                                                                    </div>
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                            <div class="flex justify-between items-center">
                                <h3 class="text-xl font-semibold text-gray-800 dark:text-gray-200 flex items-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2 text-orange-600"
                                        fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div class="overflow-y-auto max-h-[600px] p-6">
                            ...
                            @if ($inProgressMaintenances->isNotEmpty())
                                <div class="mb-6">
                                    <h4
                                        class="text-sm font-bold text-orange-700 dark:text-orange-300 mb-3 uppercase tracking-wide">
                                        In Corso
                                    </h4>
                                    @foreach ($inProgressMaintenances as $maintenance)
                                        <div
                                            class="bg-orange-50 dark:bg-orange-900/20 border-l-4 border-orange-500 rounded-lg p-4 mb-3 hover:shadow-md transition">
                            ...
                            @if ($upcomingMaintenances->isNotEmpty())
                                <div class="mb-6">
                                    ...
                                    @foreach ($upcomingMaintenances as $maintenance)
                                        <div
                                            class="bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-500 rounded-lg p-4 mb-3 hover:shadow-md transition">
                                            <div class="font-semibold text-gray-900 dark:text-gray-100">
                                                {{ $maintenance->reason }}
                                            </div>
                                            <div
                                            ...
                                            @if ($maintenance->provider)
                                                <div class="text-sm text-gray-700 dark:text-gray-300 mt-1">
                                                    Officina: <span
                                                        class="font-medium">{{ $maintenance->provider }}</span>
                                                </div>
                                            @endif
                                            <div class="mt-3 flex space-x-2">
                            ...
                            @if ($upcomingMaintenances->isEmpty() && $inProgressMaintenances->isEmpty() && $pastMaintenances->isEmpty())
                                <div class="text-center py-8">
                                    ...
                                    <p class="mt-4 text-gray-500 dark:text-gray-400 text-sm">Nessuna manutenzione
                                        registrata</p>
                                    <a href="{{ route('maintenance.create', ['vehicle_id' => $vehicle->id]) }}"

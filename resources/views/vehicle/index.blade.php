<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-xl sm:rounded-lg p-6 mb-6">
                <form method="GET" action="{{ route('vehicle.index') }}">
                    <div class="flex gap-3 items-end">
                        ...
                        <div class="flex-1">
                            <div class="relative">
                                <input
                                    ...
                                    placeholder="Cerca per targa, marca, modello o conducente..."
                                ...
                                @if(request('search'))
                                    <a href="{{ route('vehicle.index') }}" class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
                        ...
                        <div class="w-48">
                            <select
                                ...
                                <option value="unavailable" {{ request('status') == 'unavailable' ? 'selected' : '' }}>Non disponibile</option>
                            </select>
                        </div>
                ...
                @if(request()->hasAny(['search', 'brand', 'status']))
                    <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                        <p class="text-sm text-gray-600 dark:text-gray-400">
                            Trovati <span class="font-semibold text-gray-900 dark:text-gray-100">{{ $vehicles->count() }}</span>
                            {{ $vehicles->count() === 1 ? 'veicolo' : 'veicoli' }}
                        </p>
                    </div>
                @endif
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    <div class="text-gray-500 dark:text-gray-400">
                                        ...
                                        <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">Nessun veicolo trovato</h3>
                                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                            Prova a modificare i filtri di ricerca o
                                            <a href="{{ route('vehicle.index') }}" class="text-blue-500 hover:text-blue-600">resetta la ricerca</a>.
                                        </p>
                                    </div>
                                </td>
                            </tr>
    ...
    <div id="import-modal" class="hidden fixed inset-0 bg-gray-900 bg-opacity-50 z-50 flex items-center justify-center">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full mx-4">
            ...
            <!-- Modal Body -->
            <div class="px-6 py-4">
                <form action="{{ route('vehicle.import') }}" method="POST" enctype="multipart/form-data" id="import-form">
                    @csrf
                    ...
                    <div class="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                        ...
                        <ul class="text-sm text-blue-800 dark:text-blue-200 list-disc list-inside space-y-1">
                            <li><strong>plate</strong> - Targa del veicolo (obbligatoria, univoca)</li>
                            <li><strong>brand</strong> - Marca del veicolo (obbligatoria)</li>
                            <li><strong>model</strong> - Modello del veicolo (obbligatorio)</li>
                            <li><strong>status</strong> - Stato (opzionale, default: available)</li>
                        </ul>
                        ...
                        <code class="block mt-1 p-2 bg-white dark:bg-gray-900 rounded text-xs">
                            plate,brand,model,status<br>
                            AB123CD,Fiat,500,available<br>
                            EF456GH,Toyota,Yaris,available
                        </code>
                    </div>
                    ...
                    <div class="mb-4">
                        <label class="flex items-center">
                            ...
                            <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">
                                Salta targhe duplicate (se disabilitato, l'import fallirà in caso di duplicati)
                            </span>
                        </label>
                    </div>
    ...
    <script>
        ...
        function downloadTemplate() {
            const csvContent = 'plate,brand,model,status\nAB123CD,Fiat,500,available\nEF456GH,Toyota,Yaris,available';
            const blob = new Blob([csvContent], { type: 'text/csv' });
            const url = window.URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = 'template_veicoli.csv';
        ...
        document.getElementById('import-form').addEventListener('submit', function(e) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    notification.textContent = data.message || `Importati ${data.imported} veicoli con successo!`;
                    ...
                    if (data.imported > 0) {
                        ...
                        summaryLi.innerHTML = `<strong>Importati ${data.imported} veicoli con successo.</strong>`;
